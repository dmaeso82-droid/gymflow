...
class PhysicalProgressSummary extends StatelessWidget {
  ...
  IconData deltaIcon(String metric, double value) {
    if (value == 0) return Icons.remove;
    final lowerIsBetter = metric == 'bodyWeight' || metric == 'waist';
    final positiveIsGood = !lowerIsBetter;
    final good = positiveIsGood ? value > 0 : value < 0;
    return good ? Icons.trending_up : Icons.trending_down;
  }
  ...
  bool matchesUser(Map<String, dynamic> data) {
    ...
    return (userId.trim().isNotEmpty && storedUserId == userId.trim()) ||
  ...
  PhysicalMetric metricFromMaps({
    ...
    return PhysicalMetric(
      ...
      icon: icon,
      value: latestValue,
      delta: hasDelta ? latestValue - firstValue : 0,
      hasDelta: hasDelta,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final measurements = List<QueryDocumentSnapshot<Map<String, dynamic>>>.from(snapshot.data?.docs ?? [])
            .where((doc) => matchesUser(doc.data()))
            .toList();
        ...
        final metrics = [
          metricFromMaps(key: 'bodyWeight', label: 'Peso', unit: 'kg', icon: Icons.monitor_weight, latest: latest, first: first),
          metricFromMaps(key: 'waist', label: 'Cintura', unit: 'cm', icon: Icons.straighten, latest: latest, first: first),
          metricFromMaps(key: 'chest', label: 'Pecho', unit: 'cm', icon: Icons.accessibility_new, latest: latest, first: first),
          metricFromMaps(key: 'arm', label: 'Brazo', unit: 'cm', icon: Icons.fitness_center, latest: latest, first: first),
          metricFromMaps(key: 'leg', label: 'Pierna', unit: 'cm', icon: Icons.directions_run, latest: latest, first: first),
        ...
        return AppCard(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(child: PhysicalSectionHeader(title: 'Progreso físico')),
                  Container(
              ...
              SizedBox(height: 4),
              Text(
                'Última medida: ${latestDateText(latest['createdAt'])}',
...
class PhysicalSectionHeader extends StatelessWidget {
  ...
  PhysicalSectionHeader({super.key, required this.title});
    ...
    return Row(
      children: [
        Container(
          ...
          child: Icon(Icons.monitor_weight, color: context.gymFitnessAccent, size: 19),
        ),
        SizedBox(width: 10),
        Expanded(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(metric.icon, color: context.gymFitnessAccent, size: 17),
              SizedBox(width: 6),
              Expanded(
                child: Text(
                  metric.label,
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  metric.hasDelta ? signedValue(metric.delta, metric.unit) : 'Sin cambio',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(color: deltaColor, fontSize: 11, fontWeight: FontWeight.w900),
                ),
              ),
            ],
          ),
